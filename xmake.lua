...
target("cyber_engine_tweaks")
  ...
  on_install(function(target)
    ...
    if has_config("fullinstall") then
      print("Making full installation ..")
      os.mkdir("$(installpath)/cyber_engine_tweaks/scripts")
      os.cp("vendor/asiloader/*", "$(installpath)/..")
      os.cp("scripts/*", "$(installpath)/cyber_engine_tweaks/scripts")
    end
...
option("fullinstall")
  set_default(false)
  set_showmenu(true)
  set_description("Install everything (i.e. asi loader, scripts, etc...). (default: ${bright}false${clear})", "e.g.", "\t-xmake f --fullinstall=y")
